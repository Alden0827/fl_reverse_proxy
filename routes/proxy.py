...
def proxy(app_id, path):
    ...
    # Requirement 1: Protocol mismatch redirect
    # Check if any backend has a different protocol than the request
    # If so, we redirect to the first backend (or selected by round-robin)
    # regardless of online status, because we are "Opening the APP using its original URL"
    ...
    if app_item.load_balancing_enabled:
        ...
        # Note: we already incremented last_backend_index above,
        # but that was for the redirect case.
        # For proxying, we should stick to online backends.
